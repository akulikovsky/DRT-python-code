...
Initial value of the regularization parameter lamT0 can be found as following.
...
The initial guess for lampg0 should be found by the trial--and--error method.
This procedure will be improved in the next versions of the code.
...
The following plots will be shown during the code execution:
   a) The L--curve for the problem to be solved (see item 4a above)
   b) Nyquist spectrum. Points correspond to the"experimental" impedance,
      open circles show impedance reconstructed from pure Tikhonov
      regularization solution.
   c) DRT gamma(f) calculated using the Tikhonov regularization. Note
      negative non--physical oscillations of the curve.
   d) Frequency dependence of the real or imaginary part of impedance
      ...
      of fitting the curve in plot (d).
   e) Final DRT gamma(f).
   ...
   The plots above will be stored in the sub--folder './results/'
