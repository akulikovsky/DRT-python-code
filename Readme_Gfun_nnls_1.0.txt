...
This version calculates the dimensionless DRT G(f), where f is 
frequency (Hz). G(tau) satisfies to equation (Latex notations are used)

    Z(\omega) = R_{HFR}
         + R_{pol} \int_{-\infty}^{\infty} G(\tau) d \ln(\tau) / (1 + i \omega \tau)

The function G(f) is related to the DRT gamma(f) as G(f) = gamma(f) /(2*pi*f).
The method employs Tikhonov regularization in combination with 
the non--negative least squares algorithm "nnls" from
scipy.optimize library. The code seeks for vector x minimizing
...
4. To calculate DRT from your data, you have two options.
   ...
   (b) Create a text file 'MyImpedance.dat' with three columns
   separated by commas:
...
Note that for the test problem (DRT of Warburg impedance), optimal lamT = 1e-5.
